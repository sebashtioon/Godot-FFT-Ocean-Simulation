...
func _input(event: InputEvent) -> void:
	if event.is_action_pressed(&'toggle_imgui'):
		...
		var next_mode := DisplayServer.WINDOW_MODE_FULLSCREEN if DisplayServer.window_get_mode() == DisplayServer.WINDOW_MODE_WINDOWED else DisplayServer.WINDOW_MODE_WINDOWED
...
func _setup_window() -> void:
	...
	DisplayServer.window_set_position(screen_size * 0.25 / 2.0)

func _update_water_tile() -> void:
...
func _render_imgui_ocean(fps: int, mesh_quality_keys: Array) -> void:
	...
	ImGui.SameLine()
	if ImGui.SliderFloat('##update_rate', _updates_per_second, 0, 60):
...
func _render_imgui_parameters() -> void:
	ImGui.SeparatorText('Wave Parameters')
	if not ImGui.BeginTabBar('##cascades'):
		return
	for i in len(water.parameters):
		...
		if ImGui.BeginTabItem('Cascade %d' % (i + 1)):
			...
			ImGui.SameLine()
			if ImGui.SliderFloat('##displacement_scale', params._displacement_scale, 0, 2):
				params.displacement_scale = params._displacement_scale[0]
			imgui_text_tooltip('Normal Scale:      ', '')
			ImGui.SameLine()
			if ImGui.SliderFloat('##normal_scale', params._normal_scale, 0, 2):
				params.normal_scale = params._normal_scale[0]
			ImGui.Dummy(Vector2(0, 0))
			ImGui.Separator()
			ImGui.Dummy(Vector2(0, 0))
			...
			ImGui.SameLine()
			if ImGui.SliderFloat('##white_cap', params._whitecap, 0, 2):
...
func _render_imgui_camera() -> void:
	ImGui.SeparatorText('Camera')
	ImGui.Text('Camera Position:    %+.2v' % camera.global_position)
	ImGui.Text('Camera FOV:        ')
	ImGui.SameLine()
	if ImGui.SliderFloat('##fov_float', _camera_fov, 20, 170):
		camera.fov = _camera_fov[0]

func _render_imgui_footer() -> void:
	ImGui.Dummy(Vector2(0, 0))
	ImGui.Separator()
	ImGui.Dummy(Vector2(0, 0))
	ImGui.PushStyleColor(ImGui.Col_Text, Color.WEB_GRAY)
	var mod_key := 'Cmd' if OS.get_name() == 'macOS' else 'Ctrl'
	ImGui.Text('Press %s-H to toggle GUI visibility!' % [mod_key])
	ImGui.Text('Press %s-F to toggle fullscreen!' % [mod_key])
	ImGui.PopStyleColor()
